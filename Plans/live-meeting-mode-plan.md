...
**Sigurnosna posljedica koju implementacija ne smije propustiti:** izvan `(app)` **nema** nasljeđivanja `(app)/layout.tsx`-ove `maybeActor()` provjere. Zato i stranica i **svaki pojedinačni server action** režima uživo moraju sami zvati `requireActor("PRESIDENT")` (`src/server/actor.ts:8-33`, koji usput pokriva i suspendovan tenant, `:14-17`). Servisni sloj ionako čuva svoje (`requireRole` u `recordAttendance:112`, `openVoting:473`, `recordManualVote:960`), ali ruta ne smije zavisiti isključivo od toga.
...
8. **Nova ruta izvan `(app)` ostane bez zaštite**, jer ne nasljeđuje layout provjeru. *Mjera:* `requireActor("PRESIDENT")` u stranici **i u svakoj** server akciji; test po uzoru na `tests/permissions.test.ts` i `tests/tenant-isolation.test.ts`.
