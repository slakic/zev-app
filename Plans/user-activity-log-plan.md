...
zasnovano na stvarnom čitanju koda projekta, ne na pretpostavkama. Prošireno
istog dana dodatnim zahtjevom: "add ability for super admin to see all
activity for all actions, where he can filter for specific zev" — vidi §9.
...
### Kategorija `SYSTEM` — potpuno isključeno iz kuriranog prikaza (za PRESIDENT-a; vidi §9 za super admina)
...
Ovo isključenje važi samo za `/aktivnosti` (PRESIDENT, jedan tenant). Za
platformski prikaz super admina (§9) ova kategorija je upravo ono što se
traži — `admin.tenant.*` i `auth.*` su akcije od direktnog interesa na tom
nivou.
...
| `/admin/aktivnosti` (novo, cross-tenant, §9) | `requireSuperAdmin` | Van (app) sloja, u postojećem `/admin` području. Šire ovlašćenje od bilo kog drugog reda ove tabele, ali ne i nova granica povjerenja: super admin već kreira/suspenduje bilo koji ZEV nalog (`src/server/services/admin.ts`), pa uvid u aktivnost preko svih tenanata je konzistentan sa postojećim obimom te uloge. |
...
**(e) Cross-tenant vidljivost za super admina (§9) je namjerna, ne curenje.**
`/admin/aktivnosti` pokazuje aktivnost iz svih ZEV naloga jednom akteru izvan
bilo kog tenanta. Ovo je svjesno šire od svega ostalog u ovom planu, ali nije
novo proširenje ovlašćenja — super admin već upravlja svim tenantima
(kreiranje, suspenzija) sa punim pristupom bazi; ovaj prikaz mu samo daje
čitljiv uvid u ono što već arhitektonski može vidjeti.
...
### Faza 3 — super admin: pregled aktivnosti preko svih ZEV-ova (§9, dodato 2026-09-09)
14. `prisma/schema.prisma` — dodati `@@index([createdAt])` na `AuditEvent`
    (platformski upit bez `zevId` u `where`, §9).
15. Nova migracija — jedan `CREATE INDEX` iskaz.
16. `src/server/services/activity.ts` — dodati `listAllActivity(actor, {
    from, to, categories, zevId, actorUserId, page })`, gated
    `requireSuperAdmin` (ne `requireZev`); `zevId` opcioni filter; sve četiri
    kategorije uključene po default-u (uklj. `SYSTEM`). Dijeli katalog/i18n
    sloj iz Faze 2 u potpunosti.
17. `src/app/admin/aktivnosti/page.tsx` (novo) — forma filtera (period, `zev`
    `<select>` iz postojećeg `listTenants()` sa opcijom "Svi ZEV nalozi",
    kategorija, `akter` aktivan tek kad je izabran konkretan ZEV), tabela sa
    dodatnom kolonom "ZEV".
18. `src/app/admin/page.tsx` — link ka `/admin/aktivnosti` pored postojeće
    kartice "Svi ZEV nalozi".
19. `tests/admin-activity.test.ts` (novo) — `requireSuperAdmin` blokira
    ne-super-admin aktere; `zevId` filter ispravno suzi rezultate na jedan
    tenant; bez filtera vraća redove iz više tenanata uklj. one sa
    `zevId = NULL`.

### Faza 4 — opciono doterivanje (bivša Faza 3/4, prenumerisana)
20. `subjectPartyId` kolona + migracija, ako se `akter`-only (`actorId`)
    filter pokaže preuzak za praksu (§3).
21. Prikaz istorijskih glasova preko read-side spajanja sa `Vote` (§7.1).
22. Obogaćenje `issue.report` payload-a lokacijom.
23. `LEGAL_AND_FINANCIAL_ASSUMPTIONS.md` — zabilježiti odluke o
    nemogućnosti brisanja i skrivanju izbora glasa.

---

## 9. Super admin: pregled aktivnosti preko svih ZEV-ova (dodato 2026-09-09)

Naknadni zahtjev korisnika: *"add ability for super admin to see all
activity for all actions, where he can filter for specific zev."* Nije bio
pokriven originalnim planom — §0-§8 su eksplicitno vezani za jedan tenant
(`requireZev(actor)`, PRESIDENT unutar svog ZEV-a). Ovo je poseban,
platformski prikaz, provjeren naspram stvarnog `/admin` područja koje već
postoji (`src/app/admin/page.tsx`, `src/app/admin/layout.tsx`,
`src/server/services/admin.ts`).

### Zašto poseban prikaz, ne prošireni filter na `/aktivnosti`

`/aktivnosti` (§3) je izgrađen oko `requireZev(actor)` — svaki upit je tvrdo
vezan za jedan `zevId`, po dizajnu (spriječiti curenje podataka između
tenanata — isto obrazloženje kao za ostatak aplikacije,
`docs/multitenancy-plan.md` §4.4/§6). Super admin nema aktivan `zevId` —
`Actor.zevId` je `null` za "super admin bez izabranog tenanta" (komentar u
`src/server/auth/guards.ts`). Dodavanje grane "vidi sve tenante" u istu
funkciju bi značilo da tenant-scoping prestaje biti bezuslovan — tačno ona
klasa bag-a koju §0-§8 pokušava spriječiti. Umjesto toga: nova funkcija,
gated `requireSuperAdmin` (ne `requireZev`), u istom `/admin` području gdje
već žive `listTenants`/`setTenantActive` — isti arhitektonski obrazac kao i
ostatak platformskog sloja.

### Razlika naspram `/aktivnosti` — ponovna upotreba Faze 2, ne novi sloj

Katalog akcija, `labelForAction()`, `summarize()`, i18n rječnik (§2, §4) i
renderer-po-akciji logika se u potpunosti dijele sa `/aktivnosti`. Razlika je
isključivo u query obimu i guard-u:

| | `/aktivnosti` (§3, PRESIDENT) | `/admin/aktivnosti` (super admin) |
|---|---|---|
| Guard | `requireZev` (obavezan tenant) | `requireSuperAdmin` (bez tenanta) |
| `zevId` u upitu | uvijek tačno jedan (aktera) | opcioni filter — "svi" ili tačno jedan |
| Podrazumijevane kategorije | `OWNER,GOVERNANCE` | **sve četiri**, uklj. `SYSTEM` — `admin.tenant.*` i `auth.*` su upravo ono što se ovdje traži |
| `akter` filter | select članova jednog tenanta | aktivan samo kad je izabran konkretan ZEV — cross-tenant lista korisnika bez konteksta tenanta nije korisna |
| Kolona "ZEV" u tabeli | nepotrebna (uvijek isti) | **obavezna** — svaki red mora pokazati kom tenantu pripada |
| Redovi sa `zevId = NULL` | nikad se ne pojavljuju (`requireZev` ih isključuje) | **vidljivi**, "—" u koloni ZEV (npr. `auth.login` prije rezolucije tenanta) — relevantno za platformski uvid |

### Predloženi servisni potpis

```
listAllActivity(actor: Actor, {
  from, to,
  categories,       // default: sve četiri (OWNER, GOVERNANCE, FINANCE, SYSTEM)
  zevId,            // opciono — bez njega, svi tenanti
  actorUserId,      // opciono — smisleno tek kad je zevId izabran
  page,
})
```
`requireSuperAdmin(actor)` na početku umjesto `requireZev`. Oblik `where`-a
isti kao §5, samo `zevId` postaje `...(zevId ? { zevId } : {})` umjesto
obaveznog polja.

### Indeks — novi preduslov

Upit bez `zevId` filtera (izabrano "svi tenanti") skenira cijelu platformsku
`AuditEvent` tabelu sortiranu po `createdAt`. Indeksi iz §5
(`[zevId, createdAt]`, `[zevId, actorId, createdAt]`) ne pokrivaju ovaj
slučaj jer ne uključuju `zevId` u `where`. Potreban dodatni:
...
Isključivo `requireSuperAdmin` — ne PRESIDENT, ne ACCOUNTANT, bez obzira na
tenant (vidi §6 i privatnosnu napomenu (e)).

### Stranica

`src/app/admin/aktivnosti/page.tsx`, u postojećem `/admin` području (izvan
`(app)` sloja, bez `NavShell`-a — isti obrazac kao `src/app/admin/page.tsx`/
`src/app/admin/layout.tsx`). Forma filtera: period (`from`/`to`), `<select
name="zev">` popunjen iz `listTenants()` (postojeća funkcija, već se koristi
na `/admin`) sa opcijom "Svi ZEV nalozi", kategorija, i `akter` select koji
se popunjava (i postaje aktivan) tek kad je izabran konkretan ZEV. Link ka
njoj dodati na `/admin/page.tsx`, pored postojeće kartice "Svi ZEV nalozi".
...
- `src/server/services/admin.ts` (uzor za `listAllActivity` — cross-tenant, `requireSuperAdmin`, §9)
- `src/app/admin/page.tsx`, `src/app/admin/layout.tsx` (postojeće `/admin` područje gdje `/admin/aktivnosti` živi, §9)
- `src/server/auth/guards.ts` (`requireSuperAdmin`, `Actor.zevId`/`isSuperAdmin`)
